...
import InstructorAddCourses from "@/src/components/InstructorAddCourses";
import InstructorCourseCard from "@/src/components/InstructorCourseCard";
...
export default function InstructorCoursesPage() {
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            {showAddCourseForm && <div className="block p-4"><InstructorAddCourses /></div>}
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 p-4">
                {courses.length > 0 && courses.map((course) => <InstructorCourseCard key={course.id} course={course} />)}
            </div>
        </div>
    )
}